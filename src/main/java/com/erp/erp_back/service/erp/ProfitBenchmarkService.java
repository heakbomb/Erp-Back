...
public class ProfitBenchmarkService {
    ...
    public ProfitBenchmarkResponse getMonthlyProfitBenchmark(
        ...
        // ✅ 월 인건비 자동 계산
        BigDecimal myLaborAmount =
                payrollRepo.sumMonthlyLabor(storeId, payrollMonth);
        ...
        // 업계 평균(설정값)
        IndustryBenchmark bench =
                benchRepo.findBestMatch(sigungu, industry, repSubCat);
        if (bench == null) {
            bench = benchRepo.findBestMatch(null, industry, null);
        }

        BigDecimal benchCogsRate =
                bench != null ? bench.getAvgCogsRate() : BigDecimal.ZERO;
        BigDecimal benchLaborRate =
                bench != null ? bench.getAvgLaborRate() : BigDecimal.ZERO;
        Integer sampleCount =
                bench != null ? bench.getSampleCount() : 0;

        BigDecimal benchCogsAmount = sales.multiply(benchCogsRate);
        BigDecimal benchLaborAmount = sales.multiply(benchLaborRate);
        BigDecimal benchProfit =
                sales.subtract(benchCogsAmount).subtract(benchLaborAmount);
        BigDecimal benchProfitRate = rate(benchProfit, sales);

        BigDecimal diffProfit = myProfit.subtract(benchProfit);
        BigDecimal diffProfitRatePct =
                myProfitRate.subtract(benchProfitRate)
