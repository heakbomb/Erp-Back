...
public class SalesStatsService {
    ...
    public SalesSummaryResponse getSalesSummary(Long storeId) {
        ...
        Long currentMonthCountRaw = salesTransactionRepository.countByStoreStoreIdAndTransactionTimeBetween(
                storeId,
                monthRange.start(),
                LocalDateTime.now());

        Long prevMonthCountRaw = salesTransactionRepository.countByStoreStoreIdAndTransactionTimeBetween(
                storeId,
    ...
    public List<SalesDailyStatResponse> getSalesStats(Long storeId, LocalDate from, LocalDate to) {
        ...
        List<Map<String, Object>> rows = salesTransactionRepository.findDailySalesStats(storeId, range.start(),
                range.end());
        return rows.stream()
    ...
    public List<TopMenuStatsResponse> getTopMenus(Long storeId, LocalDate from, LocalDate to) {
        ...
        if (from.isBefore(today)) {
            ...
            List<TopMenuStatsResponse> pastData = salesMenuDailySummaryRepository.findTopMenuStats(storeId, from,
                    pastEnd);
    ...
    public Page<SalesTransactionSummaryResponse> getTransactions(Long storeId, LocalDate from, LocalDate to,
            Pageable pageable) {
        DateRange range = DateRangeUtils.between(from, to);
    ...
    public MenuAnalyticsResponse getMenuAnalytics(Long storeId, LocalDate from, LocalDate to) {

        LocalDate today = LocalDate.now();

        // -----------------------------
        // 1) 메뉴별 성과(Bar) = 기존 getTopMenus 재사용
        // -----------------------------
        // getTopMenus는 TopMenuStatsResponse(menuName, revenue)를 주니까
        // 그래프용으로 변환만 하면 됨.
        List<TopMenuStatsResponse> topMenus = getTopMenus(storeId, from, to);

        var menuPerformance = topMenus.stream()
                .map(r -> new MenuAnalyticsResponse.MenuPoint(
                        r.getMenuName(),
        ...
        // -----------------------------
        // 2) 카테고리별 비중(Pie) = summary(어제까지) + today(라인아이템)
        // -----------------------------
        Map<String, BigDecimal> categoryMap = new java.util.HashMap<>();

        // 2-1) 과거(요약 테이블): from ~ min(to, yesterday)
        if (from.isBefore(today)) {
            LocalDate pastEnd = to.isBefore(today) ? to : today.minusDays(1);

            var pastRows = salesMenuDailySummaryRepository.findCategorySales(storeId, from, pastEnd);
            for (var row : pastRows) {
                String cat = row.getCategory() == null ? "미분류" : row.getCategory();
                BigDecimal sales = row.getSales() == null ? BigDecimal.ZERO : row.getSales();
                categoryMap.merge(cat, sales, BigDecimal::add);
            }
        }

        // 2-2) 오늘(트랜잭션/라인아이템): 오늘 포함이면 보정
        if (!to.isBefore(today)) {
            var todayRows = salesLineItemRepository.findCategorySales(
                    storeId,
                    today.atStartOfDay(),
        ...
        var categoryData = categoryMap.entrySet().stream()
                .sorted((a, b) -> b.getValue().compareTo(a.getValue()))
                .map(e -> new MenuAnalyticsResponse.CategoryPoint(
                        e.getKey(),
                        e.getValue().setScale(0, java.math.RoundingMode.HALF_UP).longValue()))
                .toList();

        return new MenuAnalyticsResponse(menuPerformance, categoryData);
    }
}