...
public interface SalesDailySummaryRepository extends JpaRepository<SalesDailySummary, Long> {
    ...
    Optional<SalesDailySummary> findByStoreIdAndSummaryDate(Long storeId, LocalDate date);

    @Modifying
    @Query("delete from SalesDailySummary s where s.storeId = :storeId and s.summaryDate = :summaryDate")
    void deleteByStoreIdAndSummaryDate(
            @Param("storeId") Long storeId,
